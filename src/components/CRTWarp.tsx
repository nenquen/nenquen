...
float referencePlasma(vec2 uv, float t) {
  ...
  float targetAspect = 1.777; // 16:9 aspect ratio target
  
  // Map UVs so they behave like background-size: cover on a 16:9 canvas
  vec2 coverUv = uv - 0.5;
  if (screenAspect > targetAspect) {
    coverUv.y *= targetAspect / screenAspect;
  } else {
    coverUv.x *= screenAspect / targetAspect;
  }
  coverUv += 0.5;

  // Pixelation based on the 16:9 canvas
  vec2 pUv = coverUv;
  pUv *= vec2(80.0, 24.0);
  pUv = ceil(pUv);
  pUv /= vec2(80.0, 24.0);
  ...
  float field = 0.0;
  field += 0.7 * sin(0.5 * pUv.x + t / 5.0);
  field += 3.0 * sin(1.6 * pUv.y + t / 5.0);
  field += sin(10.0 * (pUv.y * sin(t / 2.0) + pUv.x * cos(t / 5.0)) + t / 2.0);

  float cx = pUv.x + 0.5 * sin(t / 2.0);
  float cy = pUv.y + 0.5 * cos(t / 4.0);
  ...
  field += 0.3 * sin(0.5 * pUv.y + pUv.x + sin(t));
